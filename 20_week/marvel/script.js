...
"id":"spiderman",
"superhero": "Спайдер-мен/Человек-паук",
"universe": "Marvel Comics",
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    for (let article of articles) {
        ...
        <h2 class="card-header">${article.superhero}</h2>
        <div class="card-body"><img src="${article.src}"></img>
        <h6 class="card-title">Вселенная: ${article.universe}</h6>
        <h6 class="card-title">Альтер эго: ${article.alter_ego}</h6>
        <h6 class="card-title">Род деятельности: ${article.occupation}</h6>
        <h6 class="card-title">Друзья: ${article.friends}</h6>
        <h6 class="card-title">Суперсилы: ${article.superpowers}</h6>
        <p class="card-text">Подробнее: ${article.more}</p>
        <div> <h5 class="card-title">Оценить супергероя</h5>
    <div class="rating-area">
	<input type="radio" id="star-5-${article.id}" name="${article.id}" value="5">
	<label for="star-5-${article.id}" title="Оценка «5»"></label>	
	<input type="radio" id="star-4-${article.id}" name="${article.id}" value="4">
	<label for="star-4-${article.id}" title="Оценка «4»"></label>    
	<input type="radio" id="star-3-${article.id}" name="${article.id}" value="3">
	<label for="star-3-${article.id}" title="Оценка «3»"></label>  
	<input type="radio" id="star-2-${article.id}" name="${article.id}" value="2">
	<label for="star-2-${article.id}" title="Оценка «2»"></label>    
	<input type="radio" id="star-1-${article.id}" name="${article.id}" value="1">
	<label for="star-1-${article.id}" title="Оценка «1»"></label>
    </div>
</div>
</div>
</div>`;
    ...
    document.getElementById("articlesContainer").innerHTML = articleContent;
